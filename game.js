...
let levelSize, ball, paddle, score, naftaliScore, michalScore, yairScore, blockCount, blob;
...
function gameInit()
{
    ...
    levelSize = vec2(72, 40); // original was 72,40
    cameraPos = levelSize.scale(.5);
    paddle = new Paddle(vec2(levelSize.x/2-12,2));
    score = michalScore = naftaliScore = yairScore = blockCount = 0;
    ...
    for (pos.y = levelSize.y/2; pos.y <= levelSize.y-4; pos.y += 2)
    {
        ...
        new Block(pos,Math.round(rand(2,4)));
    }
}
}
...
function gameUpdate()
{
    // spawn ball - infinite balls
    if ((!ball || ball) && (mouseWasPressed(0) || gamepadWasPressed(0)))
    {
...
function gameRenderPost()
{
    ...
    font.drawText(" Meirav: "+michalScore+" Yair:"+yairScore+" Naftali:"+naftaliScore, cameraPos.add(vec2(0,22)), .2, 1);
    if (!blockCount)
